...
<header class="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-40 shadow-sm">
    <div>
        ...
        <p class="text-xs text-slate-400 font-medium">Approved and HR-recommended leave records</p>
    </div>
...
<main class="flex-1 p-6 lg:p-8 bg-slate-100">
    <div class="max-w-full">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            <div>
                <a href="admin_dashboard.php" class="inline-flex items-center text-sm font-semibold text-slate-400 hover:text-blue-600 transition-colors mb-4 gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" />
                    </svg>
                    Back to Dashboard
                </a>
                <h1 class="text-3xl font-bold text-slate-800">Admin Leave History</h1>
                <p class="text-slate-500 mt-1">Detailed log of approved and pending leave records (Excluding Rejections).</p>
            </div>
            
            <form action="admin_leave_history.php" method="GET" class="flex items-center gap-3">
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            <a href="admin_leave_history.php?status=all&search=<?php echo $search_query; ?>" 
            ...
            <a href="admin_leave_history.php?status=Approved&search=<?php echo $search_query; ?>" 
                            ...
                            <tr class="hover:bg-slate-50/30 transition-colors">
                                ...
                                <td class="px-8 py-5 text-center">
                                    <?php if (!empty($row['document_path'])): ?>
                                        <a href="<?php echo $row['document_path']; ?>" target="_blank" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all inline-block">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <td class="px-8 py-5 text-sm text-slate-500">
                                    <p class="truncate max-w-[200px] italic" title="<?php echo $row['admin_remark']; ?>">
                                        <?php echo !empty($row['admin_remark']) ? htmlspecialchars($row['admin_remark']) : '<span class="text-slate-300">No Remark</span>'; ?>
                                    </p>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                            ...
                            <tr>
                                <td colspan="7" class="px-8 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-slate-400">
                                        ...
                                        <p class="font-medium">No leave records found (excluding rejections).</p>
                                    </div>
                                </td>
                            </tr>
                        <?php endif; ?>
