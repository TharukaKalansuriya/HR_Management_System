...
// Fetch Stats
$pending_label = "Awaiting Final Approval";
$pending_query_val = 'HR_Approved';
...
// Fetch Leaves for Unified Table
...
<main class="flex-grow p-6 md:p-10 bg-slate-50">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            <div>
                <h1 class="text-3xl font-bold text-slate-800">System Admin Dashboard</h1>
                <p class="text-slate-500 mt-1">Full system control and final leave oversight.</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-slate-800"><?php echo $hr_recommended_total; ?></p>
                        <h3 class="text-slate-500 text-xs font-medium uppercase tracking-wider">HR Recommended</h3>
                    </div>
                </div>
            </div>
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-slate-800"><?php echo $final_approved_count; ?></p>
                        <h3 class="text-slate-500 text-xs font-medium uppercase tracking-wider">Final Approved</h3>
                    </div>
                </div>
            </div>
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-slate-800"><?php echo $staff_count; ?></p>
                        <h3 class="text-slate-500 text-xs font-medium uppercase tracking-wider">Active Staff</h3>
                    </div>
                </div>
            </div>
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <h3 class="text-slate-500 text-xs font-medium uppercase tracking-wider">Leave Count Today</h3>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody id="leavesTableBody" class="divide-y divide-slate-50">
                                ...
                                <tr class="hover:bg-slate-50/30 transition-colors">
                                    ...
                                    <td class="px-8 py-5">
                                        <?php if (!empty($row['document_path'])): ?>
                                            <a href="<?php echo $row['document_path']; ?>" target="_blank" class="inline-flex items-center gap-1.5 text-xs font-bold text-blue-600 bg-blue-50 px-3 py-2 rounded-xl hover:bg-blue-100 transition-colors">
                                        ...
                                        <?php else: ?>
                                            <span class="text-[10px] font-bold text-slate-300 uppercase tracking-widest">No Doc</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-8 py-5">
                                        <div class="flex flex-col gap-1">
                                            <?php 
                                                ...
                                                if ($status == 'HR_Approved'): ?>
                                                    <span class="text-[10px] font-bold uppercase text-blue-500 bg-blue-50 px-2 py-0.5 rounded w-fit">HR Approved</span>
                                                <?php elseif ($status == 'Rejected'): ?>
                                                    <div class="flex flex-col gap-1">
                                                        <span class="text-[10px] font-bold uppercase text-rose-500 bg-rose-50 px-2 py-0.5 rounded w-fit">Rejected by HR</span>
                                                        <?php if (!empty($row['admin_remark'])): ?>
                                                            <div class="p-2 bg-slate-50 border-l-2 border-rose-200 rounded text-[10px] text-slate-600 italic leading-relaxed">
                                                                <span class="font-bold text-rose-400 not-italic uppercase tracking-tighter mr-1">HR Reason:</span>
                                                                <?php echo htmlspecialchars($row['admin_remark']); ?>
                                                            </div>
                                                        <?php endif; ?>
                                                    </div>
                                                <?php elseif ($status == 'Approved'): ?>
                                                    <span class="text-[10px] font-bold uppercase text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded w-fit">Approved</span>
                                                <?php endif; ?>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5">
                                        <form method="POST" class="flex items-center gap-2">
                                            ...
                                            <button type="submit" name="status" value="Approved" class="bg-emerald-500 hover:bg-emerald-600 text-white p-2 rounded-lg transition-all shadow-lg shadow-emerald-100 group" title="Approve">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="px-8 py-10 text-center text-slate-400 font-medium">No requests found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
