...
if ($status_filter === 'Approved') {
    // Show both HR_Approved and final Approved
    $query .= " AND l.status IN ('HR_Approved', 'Approved')";
} elseif ($status_filter === 'Recommended') {
...
<div class="flex flex-col flex-1 min-w-0" id="main-content">
    <header
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800">Department Leave History</h1>
            <p class="text-xs text-slate-400 font-medium">Leave records for
                <strong><?php echo htmlspecialchars($dept); ?></strong> department</p>
        </div>
        ...
        <div class="max-w-full">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    ...
                    <h1 class="text-3xl font-bold text-slate-800">Department Leave History</h1>
                    <p class="text-slate-500 mt-1">Review leave applications from your department staff.</p>
                </div>

                <form action="sup_leave_history.php" method="GET" class="flex items-center gap-3">
                    ...
                    <button type="submit"
                        class="bg-blue-600 text-white p-2.5 rounded-xl hover:bg-blue-700 transition-colors">
            ...
            <div class="flex flex-wrap gap-2 mb-8">
                <a href="sup_leave_history.php?status=all&search=<?php echo $search_query; ?>"
                ...
                <a href="sup_leave_history.php?status=Recommended&search=<?php echo $search_query; ?>"
                    class="px-5 py-2 rounded-xl text-sm font-semibold transition-all <?php echo $status_filter === 'Recommended' ? 'bg-blue-600 text-white shadow-md shadow-blue-100' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'; ?>">
                    Recommended
                </a>
                <a href="sup_leave_history.php?status=Approved&search=<?php echo $search_query; ?>"
                ...
                <a href="sup_leave_history.php?status=Pending&search=<?php echo $search_query; ?>"
                ...
                <a href="sup_leave_history.php?status=Rejected&search=<?php echo $search_query; ?>"
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50">
                                <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Employee
                                </th>
                                <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Leave
                                    Type</th>
                                <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Duration
                                </th>
                                <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Applied
                                    Date</th>
                                <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                    Documents</th>
                                <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Status
                                </th>
                                <th class="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Decision
                                    Note</th>
                            </tr>
                        </thead>
                                    ...
                                    <tr class="hover:bg-slate-50/30 transition-colors">
                                        <td class="px-8 py-5">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-9 h-9 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 font-bold text-xs border border-white shadow-sm">
                                        ...
                                        <td class="px-8 py-5">
                                            <span
                                                class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-lg uppercase tracking-wider"><?php echo str_replace('_', ' ', $row['leave_type']); ?></span>
                                        </td>
                                        <td class="px-8 py-5">
                                        ...
                                        <td class="px-8 py-5 text-sm text-slate-500">
                                            <p class="truncate max-w-[200px] italic"
                                                title="<?php echo $row['admin_remark']; ?>">
                                                <?php echo !empty($row['admin_remark']) ? htmlspecialchars($row['admin_remark']) : '<span class="text-slate-300">N/A</span>'; ?>
                                            </p>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                                ...
                                <tr>
                                    <td colspan="7" class="px-8 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-slate-400">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mb-4 opacity-20"
                                            ...
                                            <p class="font-medium">No leave records found for your department staff.</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
